...
import { useAttempts, useSubmissions, updateRecord } from '@/hooks/useSupabaseData';
import { AvatarPicker, getAvatarEmoji } from '@/components/AvatarPicker';

export default function StudentProfilePage() {
    const router = useRouter();
    const { user, logout } = useAuth();
    const { level, totalXp, streak, getUnlockedBadges, initForUser } = useGame();
    const { locale, toggleLanguage } = useLanguage();

    const { data: attempts, loading: attemptsLoading } = useAttempts({ user_id: user?.id });
    const { data: submissions, loading: submissionsLoading } = useSubmissions({ student_id: user?.id });
    ...
    const handleAvatarSelect = async (avatarId) => {
        ...
        if (user?.id) {
            try {
                await updateRecord('users', user.id, { avatar: avatarId });
            } catch (e) {
        ...
        <div className="p-4 space-y-6">
            ...
            <div className="bg-card-light rounded-xl p-4 border border-gray-100">
                ...
                <div className="space-y-3">
                    ...
                    <ProgressRow icon="📋" label="Assignments Submitted" value={stats.assignments} />
